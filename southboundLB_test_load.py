...
class OFSouthboundRequestHandler(SocketServer.StreamRequestHandler):
    ...
    def handle(self):
        ...
        print 'INFO    Setting OpenFlowRequestHandler socket from ' + str(MININET_IP) + ' to ' + str(CONTROLLERS_IP) + ':' + str(CONTROLLERS_PORT)
        OFReqForwarders.append(OpenFlowRequestForwarder(self, self.server.serverListeningPort, CONTROLLERS_IP))
        OFReqForwarders[0].start()
        try:
            while True:
                ...
                if ofop == 10:  # on PACKET_IN
                    ...
                    if address != '':
                        OF_TEST_FLOWMOD_TS.append((address, CONTROLLERS_IP, time.time()))
        ...
        OFReqForwarders[0].stop_forwarding()
